...
(function () {
   ...
   * - Forces "Your Proposal Options" selector label to render black in PDF
  ...
  function makePrintNode(node) {
    ...
    // 0b) Force the "Your Proposal Options" selector label to render black in the PDF
    try {
      const selectorLabelCandidate = Array.from(
        clone.querySelectorAll('.selector-label, h2, h3, span, div')
      ).find(
        el =>
          el.textContent &&
          el.textContent.trim().toLowerCase() === 'your proposal terms'
      );
      if (selectorLabelCandidate) {
        selectorLabelCandidate.style.color = '#28282b';
      }
    } catch (e) {
      console.warn('PDF selector label color tweak skipped:', e);
    }
